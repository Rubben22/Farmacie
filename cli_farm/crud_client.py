...
@click.option("--cnp", prompt="CNP", type=int,
              help="CNP-ul clientului")
@click.option(
    "--data-nasterii",
    prompt="Data nașterii (dd.mm.yyyy)",
    help="Data nașterii în format dd.mm.yyyy",
)
@click.pass_context
def add(ctx, id, nume, prenume, cnp, data_nasterii):
    ...
    if id in data:
        raise click.BadOptionUsage(
            "id", f"Există deja un client cu ID-ul {id}"
        )
    ...
    try:
        parsed_date = datetime.strptime(data_nasterii,
                                        "%d.%m.%Y").date()
    except ValueError:
        raise click.BadParameter(
            "Data nașterii trebuie să fie în formatul dd.mm.yyyy"
        )
...
@click.option("--data-nasterii", help="Noua data a nașterii "
                                      "(dd.mm.yyyy)")
@click.pass_context
    ...
    if cnp:
        for key, existing in data.items():
            if key != id and int(existing[ "CNP" ]) == cnp:
                raise click.BadParameter(
                    f"Exista deja un client cu CNP-ul dat."
                )
        client[ "CNP" ] = cnp
