...
def show(ctx):
    ...
    headers = [ "ID", "ID Medicament", "ID Client", "Nr Bucăți",
                "Data și Ora" ]
    click.echo(
        tabulate(tranzactii, headers=headers, tablefmt="simple_outline")
    )
...
@click.option(
    "--id-medicament",
    prompt="ID Medicament",
    help="ID-ul medicamentului vândut",
)
@click.option(
    "--id-card-client",
    prompt="ID Client (sau lasă gol dacă nu există)",
    default="",
    show_default=False,
)
@click.option(
    "--nr-bucati",
    prompt="Număr de bucăți",
    type=int,
    help="Numărul de bucăți vândute",
)
@click.option(
    "--data-ora",
    prompt="Data și ora (dd.mm.yyyy HH:MM)",
    help="Data și ora tranzacției",
)
@click.pass_context
def add(ctx, id, id_medicament, id_card_client, nr_bucati, data_ora):
    ...
    if id in data:
        raise click.BadOptionUsage(
            "id", f"Există deja o tranzacție cu ID-ul {id}"
        )
    if nr_bucati <= 0:
        raise click.BadParameter(
            "Numărul de bucăți trebuie să fie mai mare decât 0."
        )
    ...
    try:
        ...
        raise click.BadParameter(
            "Data și ora trebuie să fie în formatul dd.mm.yyyy HH:MM"
        )

    # Validate medicament exists
    if not exists_in_file(MEDICAMENT_FILE, id_medicament):
        raise click.BadParameter(
            f"Nu există niciun medicament cu ID-ul {id_medicament}"
        )
    ...
    if id_card_client.strip():
        if not exists_in_file(CLIENT_FILE, id_card_client):
            raise click.BadParameter(
                f"Nu există niciun client cu ID-ul {id_card_client}"
            )
...
@click.option("--id-card-client", help="ID nou pentru client "
                                       "(daca exista)")
@click.option("--nr-bucati", type=int, help="Număr nou de bucăți")
@click.option("--data-ora", help="Data și ora nouă "
                                 "(dd.mm.yyyy HH:MM)")
@click.pass_context
def update(ctx, id, id_medicament, id_card_client, nr_bucati, data_ora):
    ...
    if data_ora:
        try:
            parsed_datetime = datetime.strptime(data_ora,
                                                "%d.%m.%Y %H:%M")
            tranzactie[ "data_ora" ] = parsed_datetime.strftime(
                "%d.%m.%Y %H:%M")
        except ValueError:
            raise click.BadParameter(
                "Data și ora trebuie să fie în formatul dd.mm.yyyy HH:MM"
...
@operations.command(
    name="show-interval",
    help="Afișează tranzacțiile dintr-un interval de zile",
)
@click.option(
    "--de-la", required=True, help="Data de început "
                                   "(format dd.mm.yyyy)"
)
@click.option(
    "--pana-la", required=True, help="Data de sfârșit "
                                     "(format dd.mm.yyyy)"
)
...
@operations.command(
    name="top-vanzari",
    help="Afișează medicamentele ordonate descrescător după vânzări",
)
def top_vanzari():
    tranzactii = read_file(TRANZACTIE_FILE)
...
@operations.command(
    ...
    help="Șterge toate tranzacțiile dintr-un interval de zile",
)
@click.option(
    "--de-la", required=True, help="Data de început "
                                   "(format dd.mm.yyyy)"
...
def delete_interval(de_la, pana_la):
    ...
    tranzactii_filtrate = {
        k: v
        for k, v in tranzactii.items()
        if not (
                start_date
                <= datetime.strptime(v[ "data_ora" ],
                                     "%d.%m.%Y %H:%M").date()
                <= end_date
        )
    }
    ...
    click.echo(
        f"Au fost șterse {sterse} tranzacții din intervalul "
        f"{de_la} - {pana_la}."
    )
